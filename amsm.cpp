...
int usage(int ret) {
  if ((! GetConsoleWindow() || ! GetStdHandle(STD_OUTPUT_HANDLE)) && GetProcessWindowStation()) popup_message(0, MB_OK, AMSM_MESSAGE_USAGE, AMSM_VERSION, AMSM_CONFIGURATION, AMSM_DATE);
...
static int elevate(int argc, TCHAR **argv, unsigned long message) {
  ...
  if (! args) {
    print_message(stderr, AMSM_MESSAGE_OUT_OF_MEMORY, _T("GetCommandLine()"), _T("elevate()"));
    return 111;
  }

  /* Get command line, which includes the path to AMSM, and skip that part. */
...
int _tmain(int argc, TCHAR **argv) {
  ...
  if (argc > 1) {
    ...
    */
    ...
    if (str_equiv(argv[1], _T("start"))) nssm_exit(control_service(AMSM_SERVICE_CONTROL_START, argc - 2, argv + 2));
    ...
    if (str_equiv(argv[1], _T("restart"))) {
      ...
      nssm_exit(control_service(AMSM_SERVICE_CONTROL_START, argc - 2, argv + 2));
    }
    ...
    if (str_equiv(argv[1], _T("rotate"))) nssm_exit(control_service(AMSM_SERVICE_CONTROL_ROTATE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("install"))) {
      if (! is_admin) nssm_exit(elevate(argc, argv, AMSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_INSTALL));
      create_messages();
    ...
    if (str_equiv(argv[1], _T("edit")) || str_equiv(argv[1], _T("get")) || str_equiv(argv[1], _T("set")) || str_equiv(argv[1], _T("reset")) || str_equiv(argv[1], _T("unset")) || str_equiv(argv[1], _T("dump"))) {
      ...
      if (ret == 3 && ! is_admin && argc == 3) nssm_exit(elevate(argc, argv, AMSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_EDIT));
    ...
    if (str_equiv(argv[1], _T("remove"))) {
      if (! is_admin) nssm_exit(elevate(argc, argv, AMSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_REMOVE));
  ...
  */
  if (! GetStdHandle(STD_INPUT_HANDLE)) {
    /* Start service magic */
    SERVICE_TABLE_ENTRY table[] = { { AMSM, service_main }, { 0, 0 } };
    if (! StartServiceCtrlDispatcher(table)) {
      unsigned long error = GetLastError();
      /* User probably ran nssm with no argument */
      if (error == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) nssm_exit(usage(1));
      log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_DISPATCHER_FAILED, error_string(error), 0);
      nssm_exit(100);
    }
  }
