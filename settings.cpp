...
int native_set_imagepath(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (! value || ! value->string) {
    print_message(stderr, AMSM_MESSAGE_NO_DEFAULT_VALUE, name);
    return -1;
  }
...
int native_set_name(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  print_message(stderr, AMSM_MESSAGE_CANNOT_RENAME_SERVICE);
  return -1;
}
...
int native_set_objectname(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  */
  ...
  TCHAR *username = AMSM_LOCALSYSTEM_ACCOUNT;
  TCHAR *password = 0;
  if (additional) {
  ...
  if (well_known) {
    if (str_equiv(well_known, AMSM_LOCALSYSTEM_ACCOUNT)) localsystem = true;
    ...
    print_message(stderr, AMSM_MESSAGE_MISSING_PASSWORD, name);
    return -1;
  }
  ...
  */
  ...
  if (! well_known && ! virtual_account) {
    if (grant_logon_as_service(username)) {
      if (passwordsize) SecureZeroMemory(password, passwordsize);
      print_message(stderr, AMSM_MESSAGE_GRANT_LOGON_AS_SERVICE_FAILED, username);
      return -1;
    }
  }
...
int native_dump_objectname(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  if (! _tcsnicmp(AMSM_VIRTUAL_SERVICE_ACCOUNT_DOMAIN, value->string, _tcslen(AMSM_VIRTUAL_SERVICE_ACCOUNT_DOMAIN))) {
...
int native_set_startup(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  /* Map AMSM_STARTUP_* constant to Windows SERVICE_*_START constant. */
  ...
  if (service_startup < 0) {
    print_message(stderr, AMSM_MESSAGE_INVALID_SERVICE_STARTUP, value->string);
  ...
  switch (service_startup) {
    case AMSM_STARTUP_MANUAL: startup = SERVICE_DEMAND_START; break;
    case AMSM_STARTUP_DISABLED: startup = SERVICE_DISABLED; break;
  ...
  if (service_startup == AMSM_STARTUP_DELAYED) delayed.fDelayedAutostart = 1;
  ...
  if (! ChangeServiceConfig2(service_handle, SERVICE_CONFIG_DELAYED_AUTO_START_INFO, &delayed)) {
    ...
    if (error != ERROR_INVALID_LEVEL) {
      log_event(EVENTLOG_ERROR_TYPE, AMSM_MESSAGE_SERVICE_CONFIG_DELAYED_AUTO_START_INFO_FAILED, service_name, error_string(error), 0);
    }
  }
...
int native_set_type(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  */
  ...
  if (str_equiv(value->string, AMSM_INTERACTIVE_PROCESS)) type |= SERVICE_INTERACTIVE_PROCESS;
  else if (! str_equiv(value->string, AMSM_WIN32_OWN_PROCESS)) {
    print_message(stderr, AMSM_MESSAGE_INVALID_SERVICE_TYPE, value->string);
    _ftprintf(stderr, _T("%s\n"), AMSM_WIN32_OWN_PROCESS);
    _ftprintf(stderr, _T("%s\n"), AMSM_INTERACTIVE_PROCESS);
    return -1;
  }
  ...
  */
  if (type & SERVICE_INTERACTIVE_PROCESS) {
    ...
    if (! str_equiv(qsc->lpServiceStartName, AMSM_LOCALSYSTEM_ACCOUNT)) {
      HeapFree(GetProcessHeap(), 0, qsc);
      print_message(stderr, AMSM_MESSAGE_INTERACTIVE_NOT_LOCALSYSTEM, value->string, service_name, AMSM_LOCALSYSTEM_ACCOUNT);
      return -1;
    }
...
int native_get_type(const TCHAR *service_name, void *param, const TCHAR *name, void *default_value, value_t *value, const TCHAR *additional) {
  ...
  switch (value->numeric) {
    case SERVICE_KERNEL_DRIVER: string = AMSM_KERNEL_DRIVER; break;
    case SERVICE_FILE_SYSTEM_DRIVER: string = AMSM_FILE_SYSTEM_DRIVER; break;
    case SERVICE_WIN32_OWN_PROCESS: string = AMSM_WIN32_OWN_PROCESS; break;
    case SERVICE_WIN32_SHARE_PROCESS: string = AMSM_WIN32_SHARE_PROCESS; break;
    case SERVICE_WIN32_OWN_PROCESS|SERVICE_INTERACTIVE_PROCESS: string = AMSM_INTERACTIVE_PROCESS; break;
    case SERVICE_WIN32_SHARE_PROCESS|SERVICE_INTERACTIVE_PROCESS: string = AMSM_SHARE_INTERACTIVE_PROCESS; break;
    default: string = AMSM_UNKNOWN;
  }
...
int set_setting(const TCHAR *service_name, HKEY key, settings_t *setting, value_t *value, const TCHAR *additional) {
  ...
  if (! ret) print_message(stdout, AMSM_MESSAGE_RESET_SETTING, setting->name, service_name);
  else if (ret > 0) print_message(stdout, AMSM_MESSAGE_SET_SETTING, setting->name, service_name);
  else print_message(stderr, AMSM_MESSAGE_SET_SETTING_FAILED, setting->name, service_name);
...
int get_setting(const TCHAR *service_name, HKEY key, settings_t *setting, value_t *value, const TCHAR *additional) {
  ...
  if (ret < 0) print_message(stderr, AMSM_MESSAGE_GET_SETTING_FAILED, setting->name, service_name);
...
settings_t settings[] = {
  { AMSM_REG_EXE, REG_EXPAND_SZ, (void *) _T(""), false, 0, setting_set_string, setting_get_string, setting_not_dumpable },
  { AMSM_REG_FLAGS, REG_EXPAND_SZ, (void *) _T(""), false, 0, setting_set_string, setting_get_string, 0 },
  { AMSM_REG_DIR, REG_EXPAND_SZ, (void *) _T(""), false, 0, setting_set_string, setting_get_string, 0 },
  { AMSM_REG_EXIT, REG_SZ, (void *) exit_action_strings[AMSM_EXIT_RESTART], false, ADDITIONAL_MANDATORY, setting_set_exit_action, setting_get_exit_action, setting_dump_exit_action },
  { AMSM_REG_HOOK, REG_SZ, (void *) _T(""), false, ADDITIONAL_MANDATORY, setting_set_hook, setting_get_hook, setting_dump_hooks },
  { AMSM_REG_AFFINITY, REG_SZ, 0, false, 0, setting_set_affinity, setting_get_affinity, 0 },
  { AMSM_REG_ENV, REG_MULTI_SZ, NULL, false, ADDITIONAL_CRLF, setting_set_environment, setting_get_environment, setting_dump_environment },
  { AMSM_REG_ENV_EXTRA, REG_MULTI_SZ, NULL, false, ADDITIONAL_CRLF, setting_set_environment, setting_get_environment, setting_dump_environment },
  { AMSM_REG_NO_CONSOLE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_PRIORITY, REG_SZ, (void *) priority_strings[AMSM_NORMAL_PRIORITY], false, 0, setting_set_priority, setting_get_priority, setting_dump_priority },
  { AMSM_REG_RESTART_DELAY, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDIN, REG_EXPAND_SZ, NULL, false, 0, setting_set_string, setting_get_string, 0 },
  { AMSM_REG_STDIN AMSM_REG_STDIO_SHARING, REG_DWORD, (void *) AMSM_STDIN_SHARING, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDIN AMSM_REG_STDIO_DISPOSITION, REG_DWORD, (void *) AMSM_STDIN_DISPOSITION, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDIN AMSM_REG_STDIO_FLAGS, REG_DWORD, (void *) AMSM_STDIN_FLAGS, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDOUT, REG_EXPAND_SZ, NULL, false, 0, setting_set_string, setting_get_string, 0 },
  { AMSM_REG_STDOUT AMSM_REG_STDIO_SHARING, REG_DWORD, (void *) AMSM_STDOUT_SHARING, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDOUT AMSM_REG_STDIO_DISPOSITION, REG_DWORD, (void *) AMSM_STDOUT_DISPOSITION, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDOUT AMSM_REG_STDIO_FLAGS, REG_DWORD, (void *) AMSM_STDOUT_FLAGS, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDOUT AMSM_REG_STDIO_COPY_AND_TRUNCATE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDERR, REG_EXPAND_SZ, NULL, false, 0, setting_set_string, setting_get_string, 0 },
  { AMSM_REG_STDERR AMSM_REG_STDIO_SHARING, REG_DWORD, (void *) AMSM_STDERR_SHARING, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDERR AMSM_REG_STDIO_DISPOSITION, REG_DWORD, (void *) AMSM_STDERR_DISPOSITION, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDERR AMSM_REG_STDIO_FLAGS, REG_DWORD, (void *) AMSM_STDERR_FLAGS, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STDERR AMSM_REG_STDIO_COPY_AND_TRUNCATE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_STOP_METHOD_SKIP, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_KILL_CONSOLE_GRACE_PERIOD, REG_DWORD, (void *) AMSM_KILL_CONSOLE_GRACE_PERIOD, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_KILL_WINDOW_GRACE_PERIOD, REG_DWORD, (void *) AMSM_KILL_WINDOW_GRACE_PERIOD, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_KILL_THREADS_GRACE_PERIOD, REG_DWORD, (void *) AMSM_KILL_THREADS_GRACE_PERIOD, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_KILL_PROCESS_TREE, REG_DWORD, (void *) 1, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_THROTTLE, REG_DWORD, (void *) AMSM_RESET_THROTTLE_RESTART, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_HOOK_SHARE_OUTPUT_HANDLES, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_ROTATE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_ROTATE_ONLINE, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_ROTATE_SECONDS, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_ROTATE_BYTES_LOW, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_ROTATE_BYTES_HIGH, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_ROTATE_DELAY, REG_DWORD, (void *) AMSM_ROTATE_DELAY, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_REG_TIMESTAMP_LOG, REG_DWORD, 0, false, 0, setting_set_number, setting_get_number, 0 },
  { AMSM_NATIVE_DEPENDONGROUP, REG_MULTI_SZ, NULL, true, ADDITIONAL_CRLF, native_set_dependongroup, native_get_dependongroup, native_dump_dependongroup },
  { AMSM_NATIVE_DEPENDONSERVICE, REG_MULTI_SZ, NULL, true, ADDITIONAL_CRLF, native_set_dependonservice, native_get_dependonservice, native_dump_dependonservice },
  { AMSM_NATIVE_DESCRIPTION, REG_SZ, _T(""), true, 0, native_set_description, native_get_description, 0 },
  { AMSM_NATIVE_DISPLAYNAME, REG_SZ, NULL, true, 0, native_set_displayname, native_get_displayname, 0 },
  { AMSM_NATIVE_ENVIRONMENT, REG_MULTI_SZ, NULL, true, ADDITIONAL_CRLF, native_set_environment, native_get_environment, native_dump_environment },
  { AMSM_NATIVE_IMAGEPATH, REG_EXPAND_SZ, NULL, true, 0, native_set_imagepath, native_get_imagepath, setting_not_dumpable },
  { AMSM_NATIVE_OBJECTNAME, REG_SZ, AMSM_LOCALSYSTEM_ACCOUNT, true, 0, native_set_objectname, native_get_objectname, native_dump_objectname },
  { AMSM_NATIVE_NAME, REG_SZ, NULL, true, 0, native_set_name, native_get_name, setting_not_dumpable },
  { AMSM_NATIVE_STARTUP, REG_SZ, NULL, true, 0, native_set_startup, native_get_startup, 0 },
  { AMSM_NATIVE_TYPE, REG_SZ, NULL, true, 0, native_set_type, native_get_type, 0 },
