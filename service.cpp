...
int pre_edit_service(int argc, TCHAR **argv) {
  ...
  if (! (service->type & SERVICE_WIN32_OWN_PROCESS)) {
    if (mode != MODE_GETTING && mode != MODE_DUMPING) {
      ...
      CloseServiceHandle(service->handle);
      CloseServiceHandle(services);
      print_message(stderr, AMSM_MESSAGE_CANNOT_EDIT, service->name, AMSM_WIN32_OWN_PROCESS, 0);
      return 3;
    }
  }
  ...
  /* Remember the executable in case it isn't AMSM. */
  ...
  if (! service->exe[0]) {
    service->native = true;
    if (mode != MODE_GETTING && mode != MODE_DUMPING) print_message(stderr, AMSM_MESSAGE_INVALID_SERVICE, service->name, AMSM, service->image);
  }
  ...
  /* Trying to manage App* parameters for a non-AMSM service. */
  if (! setting->native && service->native) {
    CloseServiceHandle(service->handle);
    print_message(stderr, AMSM_MESSAGE_NATIVE_PARAMETER, setting->name, AMSM);
    return 1;
  }
  ...
  if (mode == MODE_RESETTING) {
    ...
    if (! value.string) {
      print_message(stderr, AMSM_MESSAGE_OUT_OF_MEMORY, _T("value"), _T("edit_service()"));
      CloseServiceHandle(service->handle);
      return 2;
    }
...
int pre_remove_service(int argc, TCHAR **argv) {
  ...
  print_message(stderr, AMSM_MESSAGE_PRE_REMOVE_SERVICE);
  return 100;
}
...
int install_service(nssm_service_t *service) {
  ...
  if (! service->handle) {
    print_message(stderr, AMSM_MESSAGE_CREATESERVICE_FAILED, error_string(GetLastError()));
    CloseServiceHandle(services);
    return 5;
  }
  ...
  print_message(stdout, AMSM_MESSAGE_SERVICE_INSTALLED, service->name);
  ...
  */
  ...
  if (service->usernamelen) {
    ...
    if (is_virtual_account(service->name, username)) {
      ...
      if (! canon) {
        print_message(stderr, AMSM_MESSAGE_OUT_OF_MEMORY, _T("canon"), _T("edit_service()"));
        return 5;
      }
  ...
  else if (editing) username = canon = AMSM_LOCALSYSTEM_ACCOUNT;
  ...
  if (service->startup == AMSM_STARTUP_DELAYED) delayed.fDelayedAutostart = 1;
  ...
  if (! ChangeServiceConfig2(service->handle, SERVICE_CONFIG_DELAYED_AUTO_START_INFO, &delayed)) {
    ...
    if (error != ERROR_INVALID_LEVEL) {
      log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_SERVICE_CONFIG_DELAYED_AUTO_START_INFO_FAILED, service->name, error_string(error), 0);
    }
  }
  ...
  if (! service->native) {
    ...
    if (create_parameters(service, editing)) {
      print_message(stderr, AMSM_MESSAGE_CREATE_PARAMETERS_FAILED);
      return 6;
    }
...
int control_service(unsigned long control, int argc, TCHAR **argv, bool return_status) {
  ...
  if (control == AMSM_SERVICE_CONTROL_START) {
    ...
    if (ret) {
      ...
      if (key) {
        if (get_number(key, AMSM_REG_THROTTLE, &cutoff, false) != 1) cutoff = AMSM_RESET_THROTTLE_RESTART;
        RegCloseKey(key);
      }
      ...
      if (response) {
        print_message(stderr, AMSM_MESSAGE_BAD_CONTROL_RESPONSE, canonical_name, service_status_text(service_status.dwCurrentState), service_control_text(control));
        if (return_status) return 0;
        return 1;
      }
...
int remove_service(nssm_service_t *service) {
  ...
  if (! DeleteService(service->handle)) {
    print_message(stderr, AMSM_MESSAGE_DELETESERVICE_FAILED);
    CloseServiceHandle(services);
    return 4;
  }
  ...
  print_message(stdout, AMSM_MESSAGE_SERVICE_REMOVED, service->name);
  return 0;
}
...
void WINAPI service_main(unsigned long argc, TCHAR **argv) {
  ...
  if (_sntprintf_s(service->name, _countof(service->name), _TRUNCATE, _T("%s"), argv[0]) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_OUT_OF_MEMORY, _T("service->name"), _T("service_main()"), 0);
    return;
  }
  ...
  service->status.dwWaitHint = AMSM_WAITHINT_MARGIN;
  ...
  service->status_handle = RegisterServiceCtrlHandlerEx(AMSM, service_control_handler, (void *) service);
  if (! service->status_handle) {
    log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_REGISTERSERVICECTRLHANDER_FAILED, error_string(GetLastError()), 0);
    return;
  }
  ...
  service->status.dwWaitHint = service->throttle_delay + AMSM_WAITHINT_MARGIN;
  ...
  if (use_critical_section) {
    ...
    if (! service->throttle_timer) {
      log_event(EVENTLOG_WARNING_TYPE, AMSM_EVENT_CREATEWAITABLETIMER_FAILED, service->name, error_string(GetLastError()), 0);
    }
  }
...
void set_service_recovery(nssm_service_t *service) {
  ...
  if (! ChangeServiceConfig2(service->handle, SERVICE_CONFIG_FAILURE_ACTIONS_FLAG, &flag)) {
    ...
    /* Pre-Vista we expect to fail with ERROR_INVALID_LEVEL */
    if (error != ERROR_INVALID_LEVEL) {
      log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_SERVICE_CONFIG_FAILURE_ACTIONS_FAILED, service->name, error_string(error), 0);
    }
  }
}
...
int monitor_service(nssm_service_t *service) {
  ...
  if (ret) {
    ...
    log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_START_SERVICE_FAILED, service->exe, service->name, ret, 0);
    return ret;
  }
  log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_STARTED_SERVICE, service->exe, service->flags, service->name, service->dir, 0);
  ...
  if (! RegisterWaitForSingleObject(&service->wait_handle, service->process_handle, end_service, (void *) service, INFINITE, WT_EXECUTEONLYONCE | WT_EXECUTELONGFUNCTION)) {
    log_event(EVENTLOG_WARNING_TYPE, AMSM_EVENT_REGISTERWAITFORSINGLEOBJECT_FAILED, service->name, service->exe, error_string(GetLastError()), 0);
  }
...
TCHAR *service_control_text(unsigned long control) {
  switch (control) {
    ...
    case AMSM_SERVICE_CONTROL_START: return _T("START");
    ...
    case AMSM_SERVICE_CONTROL_ROTATE: return _T("ROTATE");
...
void log_service_control(TCHAR *service_name, unsigned long control, bool handled) {
  ...
  if (! text) {
    ...
    if (! text) {
      log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_OUT_OF_MEMORY, _T("control code"), _T("log_service_control()"), 0);
      return;
    }
    ...
    event = AMSM_EVENT_SERVICE_CONTROL_UNKNOWN;
  }
  else if (handled) event = AMSM_EVENT_SERVICE_CONTROL_HANDLED;
  else event = AMSM_EVENT_SERVICE_CONTROL_NOT_HANDLED;

  log_event(EVENTLOG_INFORMATION_TYPE, event, service_name, text, 0);

  if (event == AMSM_EVENT_SERVICE_CONTROL_UNKNOWN) {
    HeapFree(GetProcessHeap(), 0, text);
  }
}
...
unsigned long WINAPI service_control_handler(unsigned long control, unsigned long event, void *data, void *context) {
  ...
  switch (control) {
    ...
    case SERVICE_CONTROL_STOP:
      ...
      /* Pre-stop hook. */
      nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_STOP, AMSM_HOOK_ACTION_PRE, &control, AMSM_SERVICE_STATUS_DEADLINE, false);
    ...
    case SERVICE_CONTROL_CONTINUE:
      ...
      service->status.dwWaitHint = throttle_milliseconds(service->throttle) + AMSM_WAITHINT_MARGIN;
      log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_RESET_THROTTLE, service->name, 0);
      SetServiceStatus(service->status_handle, &service->status);
      return NO_ERROR;
    ...
    case AMSM_SERVICE_CONTROL_ROTATE:
      service->last_control = control;
      log_service_control(service->name, control, true);
      (void) nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_ROTATE, AMSM_HOOK_ACTION_PRE, &control, AMSM_HOOK_DEADLINE, false);
      if (service->rotate_stdout_online == AMSM_ROTATE_ONLINE) service->rotate_stdout_online = AMSM_ROTATE_ONLINE_ASAP;
      if (service->rotate_stderr_online == AMSM_ROTATE_ONLINE) service->rotate_stderr_online = AMSM_ROTATE_ONLINE_ASAP;
      (void) nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_ROTATE, AMSM_HOOK_ACTION_POST, &control);
      return NO_ERROR;
    ...
    case SERVICE_CONTROL_POWEREVENT:
      ...
      if (event == PBT_APMRESUMEAUTOMATIC) {
        service->last_control = control;
        log_service_control(service->name, control, true);
        (void) nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_POWER, AMSM_HOOK_ACTION_RESUME, &control);
        return NO_ERROR;
      }
      ...
      if (event == PBT_APMPOWERSTATUSCHANGE) {
        service->last_control = control;
        log_service_control(service->name, control, true);
        (void) nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_POWER, AMSM_HOOK_ACTION_CHANGE, &control);
        return NO_ERROR;
      }
      log_service_control(service->name, control, false);
      return NO_ERROR;
...
int start_service(nssm_service_t *service) {
  ...
  if (ret) {
    log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_GET_PARAMETERS_FAILED, service->name, 0);
  ...
  if (_sntprintf_s(cmd, _countof(cmd), _TRUNCATE, _T("\"%s\" %s"), service->exe, service->flags) < 0) {
    log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_OUT_OF_MEMORY, _T("command line"), _T("start_service"), 0);
    unset_service_environment(service);
    return stop_service(service, 2, true, true);
  }
  ...
  unsigned long control = AMSM_SERVICE_CONTROL_START;
  ...
  if (service->allow_restart) {
    ...
    if (nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_START, AMSM_HOOK_ACTION_PRE, &control, AMSM_SERVICE_STATUS_DEADLINE, false) == AMSM_HOOK_STATUS_ABORT) {
      TCHAR code[16];
      _sntprintf_s(code, _countof(code), _TRUNCATE, _T("%lu"), AMSM_HOOK_STATUS_ABORT);
      log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_PRESTART_HOOK_ABORT, AMSM_HOOK_EVENT_START, AMSM_HOOK_ACTION_PRE, service->name, code, 0);
      unset_service_environment(service);
    ...
    if (! CreateProcess(0, cmd, 0, 0, inherit_handles, flags, 0, service->dir, &si, &pi)) {
      ...
      unsigned long error = GetLastError();
      log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_CREATEPROCESS_FAILED, service->name, service->exe, error_string(error), 0);
      close_output_handles(&si);
      unset_service_environment(service);
    ...
    if (service->affinity) {
      /*
        ...
        (or when running the 32-bit AMSM).
      ...
      */
      ...
      if (GetProcessAffinityMask(service->process_handle, &affinity, &system_affinity)) affinity = service->affinity & system_affinity;
        ...
        log_event(EVENTLOG_ERROR_TYPE, AMSM_EVENT_GETPROCESSAFFINITYMASK_FAILED, service->name, error_string(GetLastError()), 0);
      }

      if (! SetProcessAffinityMask(service->process_handle, affinity)) {
        log_event(EVENTLOG_WARNING_TYPE, AMSM_EVENT_SETPROCESSAFFINITYMASK_FAILED, service->name, error_string(GetLastError()), 0);
      }
  ...
  */
  ...
  /* Post-start hook. */
  if (! service->throttle) {
    (void) nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_START, AMSM_HOOK_ACTION_POST, &control);
  }
...
int stop_service(nssm_service_t *service, unsigned long exitcode, bool graceful, bool default_action) {
  ...
  service->rotate_stdout_online = service->rotate_stderr_online = AMSM_ROTATE_OFFLINE;

  if (default_action && ! exitcode && ! graceful) {
    log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_GRACEFUL_SUICIDE, service->name, service->exe, exit_action_strings[AMSM_EXIT_UNCLEAN], exit_action_strings[AMSM_EXIT_UNCLEAN], exit_action_strings[AMSM_EXIT_UNCLEAN], exit_action_strings[AMSM_EXIT_REALLY], 0);
    graceful = true;
  }
  ...
  if (service->pid) {
    /* Shut down service */
    log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_TERMINATEPROCESS, service->name, service->exe, 0);
    kill_t k;
    service_kill_t(service, &k);
    k.exitcode = 0;
    kill_process(&k);
  }
  else log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_PROCESS_ALREADY_STOPPED, service->name, service->exe, 0);
...
void CALLBACK end_service(void *arg, unsigned char why) {
  ...
  */
  if (! why) {
    ...
    log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_ENDED_SERVICE, service->exe, service->name, code, 0);
  }
  ...
  /* Exit hook. */
  service->exit_count++;
  (void) nssm_hook(&hook_threads, service, AMSM_HOOK_EVENT_EXIT, AMSM_HOOK_ACTION_POST, NULL, AMSM_HOOK_DEADLINE, true);
  ...
  */
  ...
  switch (action) {
    ...
    case AMSM_EXIT_RESTART:
      log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_EXIT_RESTART, service->name, code, exit_action_strings[action], service->exe, 0);
      while (monitor_service(service)) {
        log_event(EVENTLOG_WARNING_TYPE, AMSM_EVENT_RESTART_SERVICE_FAILED, service->exe, service->name, 0);
        Sleep(30000);
      }
    break;

    /* Do nothing, just like srvany would */
    case AMSM_EXIT_IGNORE:
      log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_EXIT_IGNORE, service->name, code, exit_action_strings[action], service->exe, 0);
      wait_for_hooks(service, false);
      Sleep(INFINITE);
    break;

    /* Tell the service manager we are finished */
    case AMSM_EXIT_REALLY:
      log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_EXIT_REALLY, service->name, code, exit_action_strings[action], 0);
      stop_service(service, exitcode, true, default_action);
    break;

    /* Fake a crash so pre-Vista service managers will run recovery actions. */
    case AMSM_EXIT_UNCLEAN:
      log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_EXIT_UNCLEAN, service->name, code, exit_action_strings[action], 0);
...
void throttle_restart(nssm_service_t *service) {
  ...
  if (service->throttle == 1 && service->restart_delay > throttle_ms) log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_RESTART_DELAY, service->name, milliseconds, 0);
  else {
    _sntprintf_s(threshold, _countof(threshold), _TRUNCATE, _T("%lu"), service->throttle_delay);
    log_event(EVENTLOG_WARNING_TYPE, AMSM_EVENT_THROTTLED, service->name, threshold, milliseconds, 0);
  }
...
/*
  ...
  AMSM_SERVICE_STATUS_DEADLINE milliseconds.  If the process is still running
  and we haven't finished waiting we increment dwCheckPoint and add whichever is
  smaller of AMSM_SERVICE_STATUS_DEADLINE or the remaining timeout to
  dwWaitHint.
...
int await_single_handle(SERVICE_STATUS_HANDLE status_handle, SERVICE_STATUS *status, HANDLE handle, TCHAR *name, TCHAR *function_name, unsigned long timeout) {
  ...
  while (waited < timeout) {
    ...
    if (interval > AMSM_SERVICE_STATUS_DEADLINE) interval = AMSM_SERVICE_STATUS_DEADLINE;
    ...
    if (waited) {
      ...
      log_event(EVENTLOG_INFORMATION_TYPE, AMSM_EVENT_AWAITING_SINGLE_HANDLE, function, name, waited_milliseconds, interval_milliseconds, timeout_milliseconds, 0);
    }
...
int list_nssm_services(int argc, TCHAR **argv) {
  ...
  ENUM_SERVICE_STATUS_PROCESS *status = (ENUM_SERVICE_STATUS_PROCESS *) HeapAlloc(GetProcessHeap(), 0, required);
  if (! status) {
    print_message(stderr, AMSM_MESSAGE_OUT_OF_MEMORY, _T("ENUM_SERVICE_STATUS_PROCESS"), _T("list_nssm_services()"));
    return 3;
  }
  ...
  while (true) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (! service) {
        HeapFree(GetProcessHeap(), 0, status);
        print_message(stderr, AMSM_MESSAGE_OUT_OF_MEMORY, _T("nssm_service_t"), _T("list_nssm_services()"));
        return 5;
      }
